...
relative cost of using STM. However, this situation isn't a typical use of
Apache's cache.

\begin{figure*}
 \centering
 \subfigure[Request thorughput]{
...
the decrease in performance caused by STM was even less significant. [XX Do the
single machine graphs contribute anything? XX]
...
\begin{figure*}
 ...
 \caption{Single host, $s = 1$: Medium locality.
          With a single host, the additional latency of the network is
          eliminated, achieving generally higher results than with two hosts. [XX should we keep these? XX]}
\end{figure*}
...
\begin{figure*}
 \centering
 \subfigure[Request throughput. Without the additional processes apache fails
            to scale with high number of cores. STM suffers a decrease in
            performance but is comparable.]{
  \includegraphics[width=8cm]{transaction-rate-single-process.png}
  \label{fig:one-process-request-throughput}
 }
 \hfill
 \subfigure[Abort rate. The abort rate increase with the number of cores might
            explain the decrease in transaction rate.]{
