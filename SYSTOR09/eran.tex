...
\alignauthor
Haggai Eran\\
       \email{haggaie@tx.technion.ac.il}
\alignauthor
Ohad Lutzky\\
...
experience in modifying Apache's cache module to employ transactional memory instead of
...
\emph{transactification}. Specifically, we transactify the cache module of Apache 
web server.

When approaching a transactifyication process of such magnitude (Apache is
comprised of about 340,000 lines of code), one must focus the effort, in order
to complete the transactifyication in smaller and modular changes, where the
majority of the code base is left untouched. Therefore our first step in the
process was a top level analysis of Apache's code base, looking for
transactional memory opportunities. We identified Apache's cache module as an
interesting target for transactifycation, that was compatible with our goals.

Since Apache is written in C, we needed to employ C-based
...
Apache uses locks for synchronization in quite a few places of its code, when
running in a multi-threaded setup. Locks are used by the worker MPM,
to manages access to queues of incoming requests, they are used by the memory
management system for allocating buffers and also to synchronize access to
buffered I/O. In addition, locks are used under some of Apache's extension
modules such as {\tt mod\_dbd}, {\tt mod\_ldap}, {\tt mod\_rewrite}, {\tt
mod\_proxy} and the {\tt mod\_cache} caching module.
...
It is interesting to note that the new transactions contained in most cases both
read and write access to shared memory, both in case of a cache miss, where a
new object is added to the cache, and in case of a cache hit, when the
bookkeeping information of the requested object is updated.
...
We assume that this is caused by the fact that the cache is not shared between
processes, so by running apache with multiple processes the cache capacity is
virtually reduced. Nevertheless, as before, neither the transactified version
nor the original version was able to benefit from the maximum allowed
